...
export class RoboflowAPIObjectDetector {
  public static loadModel(
    ...
    if (activeLabelType === LabelType.RECT) {
      ...
      const wrappedOnFailure = () => {
        store.dispatch(
          updateRoboflowAPIDetails({ status: false, model: "", key: "" })
        );
        onFailure();
      };
  ...
  public static predict(
    ...
    const roboflowAPIDetails: RoboflowAPIDetails =
      AISelector.getRoboflowAPIDetails();
      ...
      axios({
        ...
        .then(response => {
          const predictions: DetectedObject[] = (
            response.data.predictions as RoboflowPrediction[]
