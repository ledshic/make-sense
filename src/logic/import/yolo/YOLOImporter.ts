...
export class YOLOImporter extends AnnotationImporter {
  ...
  public import(
    ...
    try {
      ...
      const { labelNameFile, annotationFiles } = YOLOImporter.filterFilesData(
        filesData,
        sourceImagesData
      );
      const [relevantImageData, relevantAnnotations] =
        YOLOImporter.matchImagesWithAnnotations(
          sourceImagesData,
          annotationFiles
        );
      const labelNamesPromise: Promise<LabelName[]> = FileUtil.readFile(
        ...
        YOLOUtils.parseLabelsNamesFromString(fileContent)
      );
      ...
      Promise.all([
        ...
        .then((values: [LabelName[], void, string[]]) => {
          ...
          const resultImageData = zip<ImageData, string>(
            ...
            YOLOImporter.applyAnnotations(pair[0], pair[1], labelNames)
          );
          onSuccess(
            YOLOImporter.injectImageDataWithAnnotations(
              sourceImagesData,
              resultImageData
            ),
            labelNames
