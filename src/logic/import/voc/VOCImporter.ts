...
export class VOCImporter extends AnnotationImporter {
  public import(
    ...
    try {
      const inputImagesData: Record<string, ImageData> =
        VOCImporter.mapImageData();
  ...
  private loadAndParseFiles(files: File[]): Promise<VOCImportResult> {
    return Promise.all(files.map((file: File) => file.text())).then(
      (fileTexts: string[]) =>
        fileTexts.reduce(
          (
            current: VOCImportResult,
            fileText: string,
            currentIndex: number
          ) => {
            ...
            try {
              ...
              if (e instanceof DocumentParsingError) {
                throw new DocumentParsingError(
                  `Failed trying to parse ${fileName} as VOC XML document.`
                );
  ...
  protected static parseDocumentIntoImageData(
    ...
    try {
      ...
      const [labeledBoxes, newLabelNames] = this.parseAnnotationsFromFileString(
        document,
        labelNames
      );
  ...
  protected static parseAnnotationsFromFileString(
    ...
    const newLabelNames: Record<string, LabelName> = Object.assign(
      {},
      labelNames
    );
    return [
