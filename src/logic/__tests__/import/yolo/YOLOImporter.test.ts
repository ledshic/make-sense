...
describe("YOLOImporter filterFilesData method", () => {
  it("should return correct fileData partition", () => {
    ...
    const imagesData: ImageData[] = imageFileNames.map((fileName: string) =>
      getDummyImageData(fileName)
    );
    ...
    const annotationFiles: File[] = annotationFileNames.map(
      (fileName: string) => getDummyFileData(fileName)
    );
    const labelFileNames = ["labels.txt"];
    const labelFiles: File[] = labelFileNames.map((fileName: string) =>
      getDummyFileData(fileName)
    );
