...
class TestableVOCImporter extends VOCImporter {
  public static testableParseAnnotationsFromFileString(
    ...
    return TestableVOCImporter.parseAnnotationsFromFileString(
      document,
      labelNames
    );
  }
...
describe("VOCImporter parseAnnotationsFromFileString method", () => {
  it("should return correctly for multiple annotations", () => {
    ...
    // when
    const [annotations, newRecordSet] =
      TestableVOCImporter.testableParseAnnotationsFromFileString(
        validTestDocument,
        emptyRecordSet
      );
  ...
  it("should reuse existing labels", () => {
    ...
    expect(newRecordSet).toEqual(
      expect.objectContaining({
        annotation1: expect.objectContaining({ name: "annotation1" }),
        annotation2: expect.objectContaining({
          name: "annotation2",
          id: "foobar",
        }),
      })
