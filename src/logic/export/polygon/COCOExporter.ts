...
export class COCOExporter {
  public static export(): void {
    ...
    const COCOObject: COCOObject = COCOExporter.mapImagesDataToCOCOObject(
      imagesData,
      labelNames,
      projectName
    );
  ...
  public static getCategoriesComponent(
    labelNames: LabelName[]
  ): COCOCategory[] {
      ...
      .map((imageData: ImageData, index: number) => {
        return imageData.labelPolygons.map((labelPolygon: LabelPolygon) => {
          return {
            ...
            segmentation: COCOExporter.getCOCOSegmentation(
  ...
  public static mapLabelsData(labelNames: LabelName[]): LabelDataMap {
    return labelNames.reduce(
      (data: LabelDataMap, label: LabelName, index: number) => {
  ...
  public static getCOCOSegmentation(vertices: IPoint[]): COCOSegmentation {
    const points: number[][] = vertices.map((point: IPoint) => [
      point.x,
      point.y,
    ]);
