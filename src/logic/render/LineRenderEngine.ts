...
export class LineRenderEngine extends BaseRenderEngine {
  ...
  public mouseDownHandler(data: EditorData): void {
    ...
    if (isMouseOverCanvas) {
      if (!!anchorTypeUnderMouse && !this.isResizeInProgress()) {
        this.startExistingLabelUpdate(
          labelLineUnderMouse.id,
          anchorTypeUnderMouse
        );
    ...
    imageData.labelLines.forEach((labelLine: LabelLine) => {
      if (labelLine.isVisible) {
        const isActive: boolean =
          labelLine.id === activeLabelId || labelLine.id === highlightedLabelId;
        const lineOnCanvas =
  ...
  private drawActivelyCreatedLabel(data: EditorData) {
    if (this.isInProgress()) {
      const line = {
        start: this.lineCreationStartPoint,
        end: data.mousePositionOnViewPortContent,
      };
      DrawUtil.drawLine(
        this.canvas,
        line.start,
  ...
  private startNewLabelCreation = (data: EditorData) => {
    this.lineCreationStartPoint = RenderEngineUtil.setPointBetweenPixels(
      data.mousePositionOnViewPortContent
    );
  ...
  private finishNewLabelCreation = (data: EditorData) => {
    ...
    const lineOnCanvas = {
      start: this.lineCreationStartPoint,
      end: mousePositionOnCanvasSnapped,
    };
    const lineOnImage = RenderEngineUtil.transferLineFromViewPortContentToImage(
      lineOnCanvas,
      data
    );
  ...
  private startExistingLabelUpdate(
    labelId: string,
    anchorType: LineAnchorType
  ) {
  ...
  private getLineUnderMouse(data: EditorData): LabelLine | null {
    ...
    const labelLines: LabelLine[] =
      LabelsSelector.getActiveImageData().labelLines.filter(
    ...
    for (const labelLine of labelLines) {
      const lineOnCanvas: ILine =
        RenderEngineUtil.transferLineFromImageToViewPortContent(
          labelLine.line,
          data
        );
      if (RenderEngineUtil.isMouseOverLine(mouseOnCanvas, lineOnCanvas, radius))
        return labelLine;
    }
    return null;
  }
