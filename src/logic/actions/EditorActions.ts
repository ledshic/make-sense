...
export class EditorActions {
  ...
  public static mountSupportRenderingEngine(activeLabelType: LabelType) {
    switch (activeLabelType) {
      case LabelType.RECT:
        EditorModel.supportRenderingEngine = new RectRenderEngine(
          EditorModel.canvas
        );
        break;
      case LabelType.POINT:
        EditorModel.supportRenderingEngine = new PointRenderEngine(
          EditorModel.canvas
        );
        break;
      case LabelType.LINE:
        EditorModel.supportRenderingEngine = new LineRenderEngine(
          EditorModel.canvas
        );
        break;
      case LabelType.POLYGON:
        EditorModel.supportRenderingEngine = new PolygonRenderEngine(
          EditorModel.canvas
        );
        break;
      default:
  ...
  public static mountRenderEnginesAndHelpers(activeLabelType: LabelType) {
    ...
    EditorModel.primaryRenderingEngine = new PrimaryEditorRenderEngine(
      EditorModel.canvas
    );
    EditorActions.mountSupportRenderingEngine(activeLabelType);
  }
  ...
  public static getEditorData(event?: Event): EditorData {
    return {
      ...
      absoluteViewPortContentScrollPosition:
        ViewPortActions.getAbsoluteScrollPosition(),
    };
  }
  ...
  public static updateMousePositionIndicator(
    ...
    const mousePositionOverViewPortContent: IPoint =
      CanvasUtil.getMousePositionOnCanvasFromEvent(event, EditorModel.canvas);
    const viewPortContentScrollPosition: IPoint =
      ViewPortActions.getAbsoluteScrollPosition();
    const viewPortContentImageRect: IRect =
      ViewPortActions.calculateViewPortContentImageRect();
    ...
    if (
      isMouseOverViewPort &&
      !GeneralSelector.getPreventCustomCursorStatus()
    ) {
      ...
      if (
        isMouseOverImage &&
        ![CustomCursorStyle.GRAB, CustomCursorStyle.GRABBING].includes(
          GeneralSelector.getCustomCursorStyle()
        )
      ) {
        ...
        EditorModel.mousePositionIndicator.style.left =
          mousePositionOverViewPort.x + 15 + "px";
        EditorModel.mousePositionIndicator.style.top =
          mousePositionOverViewPort.y + 15 + "px";
