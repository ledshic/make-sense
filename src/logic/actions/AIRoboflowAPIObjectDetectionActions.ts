...
import {
  DetectedObject,
  RoboflowAPIObjectDetector,
} from "../../ai/RoboflowAPIObjectDetector";
...
export class AIRoboflowAPIObjectDetectionActions {
  public static detectRects(imageData: ImageData): void {
    if (
      ...
      !AISelector.isRoboflowAPIModelLoaded()
    )
      return;

    store.dispatch(updateActivePopupType(PopupWindowType.LOADER));
    RoboflowAPIObjectDetector.predict(
      imageData,
      (predictions: DetectedObject[]) => {
        const suggestedLabelNames =
          AIRoboflowAPIObjectDetectionActions.extractNewSuggestedLabelNames(
        ...
        AIRoboflowAPIObjectDetectionActions.saveRectPredictions(
          imageData,
          predictions
        );
      }
  ...
  public static saveRectPredictions(
    ...
    const predictedLabels: LabelRect[] =
      AIRoboflowAPIObjectDetectionActions.mapPredictionsToRectLabels(
        predictions
      );
