...
export class VirtualListUtil {
  ...
  public static calculateAnchorPoints(
    listSize: ISize,
    childSize: ISize,
    childCount: number
  ): IPoint[] {
    const gridSize: ISize = VirtualListUtil.calculateGridSize(
      listSize,
      childSize,
      childCount
    );
    const contentWrapperSize: ISize = VirtualListUtil.calculateContentSize(
      listSize,
      childSize,
      gridSize
    );
    const horizontalMargin =
      (contentWrapperSize.width - gridSize.width * childSize.width) /
      (gridSize.width + 1);
