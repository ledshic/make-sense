...
const PretrainedModelDataMap: Record<
  PretrainedModel,
  IPretrainedModelSpecification
> = {
...
const LoadYOLOv5ModelPopup: React.FC<IProps> = ({
  updateActivePopupTypeAction,
  submitNewNotificationAction,
}) => {
  // BUSINESS LOGIC
  ...
  const onDrop = (accepted: File[]) => {
    const jsonFiles = accepted.filter((file: File) =>
  ...
  const onAccept = () => {
    ...
    const onFailure = () => {
      setIsLoading(false);
      const notification =
        modelSource === ModelSource.UPLOAD
    ...
    if (modelSource === ModelSource.DOWNLOAD) {
      ...
      const config = {
        source: modelFiles,
        classNames: classNames.map((className: LabelName) => className.name),
      };
  ...
  const renderMessage = () => {
    ...
    const downloadMessage: string =
      "Use one of ours pretrained YOLOv5 models to speed up annotation process.";
    return (
  ...
  const renderContent = () => {
    const shouldRenderDropZone =
      !isLoading && modelSource === ModelSource.UPLOAD;
    const shouldRenderOptions =
      !isLoading && modelSource === ModelSource.DOWNLOAD;
    return (
