...
const SuggestLabelNamesPopup: React.FC<IProps> = ({
  ...
  const [labelNames, setLabelNames] = useState(
    mapNamesToSelectableNames(AISelector.getSuggestedLabelList())
  );

  const onAccept = () => {
    updateLabelNames(
      extractSelectedNames().reduce(
        (acc: LabelName[], entry: string, index: number) => {
          acc.push({
            name: entry,
            id: uuidv4(),
    ...
    updateRejectedSuggestedLabelList(
      AISelector.getRejectedSuggestedLabelList().concat(
  ...
  const onReject = () => {
    updateRejectedSuggestedLabelList(
      AISelector.getRejectedSuggestedLabelList().concat(extractNames())
    );
    updateSuggestedLabelList([]);
    PopupActions.close();
  };
  ...
  const toggleSelectableNameByIndex = (index: number) => {
    const nextLabelNames: SelectableName[] = labelNames.map(
      (entry: SelectableName, entryIndex: number) => {
        if (index === entryIndex)
          return {
            ...entry,
            flag: !entry.flag,
    ...
    const nextSelectAllFlag: boolean = nextLabelNames.reduce(
      (acc: boolean, entry: SelectableName) => {
    ...
    return (
      <div className="SuggestLabelNamesPopupContent">
        <div className="Message">
          We found objects of classes that are not yet included in the list of
          labels. Select the names you would like to add. This will help to
