...
const ConnectInferenceServerPopup: React.FC<IProps> = ({
  ...
  // general
  const [currentServerType, setCurrentServerType] = useState(
  ...
  const roboflowModelOnChangeCallback = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    setRoboflowModel(event.target.value);
  };

  const roboflowKeyOnChangeCallback = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    ...
    return (
      <>
        <div className="message">
          Provide details of the Roboflow model you want to run over tha API, as
          well as your API key.
        </div>
  ...
  const renderSideMenuContent = (): JSX.Element[] => {
    return Object.entries(InferenceServerDataMap).map(
      ([serverType, serverData], index: number) => {
        return (
          <ImageButton
            key={index}
