...
const StateBar: React.FC<IProps> = ({ imagesData, activeLabelType }) => {
  const pointLabeledImages = imagesData.reduce(
  ...
  const rectLabeledImages = imagesData.reduce(
    (currentCount: number, currentImage: ImageData) => {
  ...
  const polygonLabeledImages = imagesData.reduce(
    (currentCount: number, currentImage: ImageData) => {
  ...
  const lineLabeledImages = imagesData.reduce(
    (currentCount: number, currentImage: ImageData) => {
  ...
  const tagLabeledImages = imagesData.reduce(
    (currentCount: number, currentImage: ImageData) => {
